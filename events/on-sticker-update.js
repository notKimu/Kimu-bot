...
module.exports = {
    ...
    async execute(oldSticker, sticker) {
        // Variables
        ...
        getLogChannel().then(async channelSetted => {
            ...
            guild.fetchAuditLogs().then(async audit => {
                ...
                const stickerCreate = new EmbedBuilder()
                    .setColor('#66cdcc')
                    .setTitle(`The sticker **${oldSticker.name}** was updated!`)
            ...
            }).catch(err => console.log("Error on updated sticker log => " + err));

        }).catch(err => console.log("Error on updated sticker => " + err));
    }
}